    ...
    window.showToast = function(message, type = 'info', title = '', duration = 5000) {
        ...
        switch(type) {
            ...
            case 'error':
                ...
                title = title || '{{ __("general.error") }}';
                break;
            case 'warning':
                icon.classList.add('fa-exclamation-triangle');
                title = title || '{{ __("general.warning") }}';
                break;
            case 'info':
                icon.classList.add('fa-info-circle');
                title = title || '{{ __("general.info") }}';
                break;
        }
    ...
    if (typeof $ !== 'undefined') {
        $(document).ajaxError(function(event, jqXHR, ajaxSettings, thrownError) {
            if (jqXHR.status === 419) {
                ...
                showToast('{{ __("validation.unauthorized") }}', 'error', '{{ __("general.error") }}');
            } else if (jqXHR.status === 403) {
                showToast('{{ __("validation.forbidden") }}', 'error', '{{ __("general.error") }}');
            } else if (jqXHR.status === 404) {
                showToast('{{ __("validation.not_found") }}', 'error', '{{ __("general.error") }}');
            } else if (jqXHR.status === 422) {
                // Validation errors
                if (jqXHR.responseJSON && jqXHR.responseJSON.errors) {
                    const errors = Object.values(jqXHR.responseJSON.errors).flat();
                    showToast(errors.join('<br>'), 'error', '{{ __("validation.error_title") }}');
                }
            } else if (jqXHR.status >= 500) {
                showToast('{{ __("validation.server_error") }}', 'error', '{{ __("general.error") }}');
            }
        });
    }
</script>
