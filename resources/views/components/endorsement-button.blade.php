...
Professional "Recommend" button with Alpine.js for instant UI update

Usage: <x-endorsement-button :service-provider="$serviceProvider" />
--}}
...
<div x-data="{
        endorsed: {{ $isEndorsed ? 'true' : 'false' }},
        ...
        async toggle() {
            ...
            if (!this.isClient) {
                // Show toast message
                this.$dispatch('toast', { message: '{{ __('endorsements.clients_only') }}', type: 'warning' });
                return;
            }
            ...
            try {
                const response = await fetch('{{ route('endorsements.toggle', $serviceProvider->id) }}', {
                    method: 'POST',
                    headers: {
                ...
                this.$dispatch('toast', { message: '{{ __('general.error_occurred') }}', type: 'error' });
            } finally {
                this.loading = false;
            }
        }
    ...
    <button @click="toggle()" :disabled="loading" :class="{
        ...
        :title="endorsed ? '{{ __('endorsements.recommended') }}' : '{{ __('endorsements.recommend') }}'"
        aria-label="{{ __('endorsements.recommend') }}">
        {{-- Loading spinner --}}
        ...
        <span x-show="!loading" class="endorsement-icon">
            {{-- Heroicon: hand-thumb-up (outline when not endorsed, solid when endorsed) --}}
        ...
        <span class="endorsement-label"
            x-text="endorsed ? '{{ __('endorsements.recommended') }}' : '{{ __('endorsements.recommend') }}'"></span>
